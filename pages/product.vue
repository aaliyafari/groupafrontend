...
async function getProductAPI() {
  mydata.allProduct = await $fetch("http://localhost:3001/feed/allData");
}
//POST API
...
async function onFormSubmit1() {
  try {
    ...
    await $fetch("http://localhost:3001/feed/create", {
      method: "POST",
      body: JSON.stringify(mydata.product),
    });
...
async function editProduct(id) {
  ...
  const response = await $fetch("http://localhost:3001/feed/patch/" + id, {
    method: "PATCH",
    body: JSON.stringify(mydata.product),
  });
  getProductAPI();
...
async function onDeleteOfProduct(id) {
  await $fetch("http://localhost:3001/feed/" + id, {
    method: "DELETE",
  });
  getProductAPI();
}
